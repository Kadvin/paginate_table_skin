...
//Paginate.Page
Paginate.Page = Class.create({
  initialize: function(element) {
    //根据构造参数初始化一些变量
    this.element = $(element);
    this.base_url = this.element.readAttribute("url");
    // Options是放在 div.paginate里面额外的属性
    var options = Object.keys(Paginate.Options).inject({}, function(acc,attr){
      var option = element.readAttribute(attr);
      if( option != null ) acc[attr] = option;
      return acc;
    });
    this.options = Paginate.Options;
   ...
   //初始化组成对象
    ...
    var domTable = this.element.select("div.paginate_table").first();
    var domStatusbar = this.element.select("table.statusbar").first();
    this.toolbar = new Paginate.Toolbar(this, domToolbar);
    this.table = new Paginate.Table(this, domTable);
    this.statusbar = new Paginate.Statusbar(this, domStatusbar);
    this.adjustHeight();
  ...
  fire : function(eventName, memo){
    Event.fire(this.element, "paginate:" + this.element.id + ":" + eventName, memo)
  },
  ...
  observe : function(eventName, observer, target){
    Event.observe(this.element, "paginate:"+ this.element.id + ":" + eventName, observer)
  },
  ...
  hookEventListeners : function(item){
    ...
    $w("onSelectionChanged onFetchSuccess onFetchFailure onFetchComplete").each(function(eventName){
      ...
      if( target ) {
        ...
        page.observe(event, Paginate.Page.wrapEventListener(item, target))
      }
    })
  },
  ...
  openDetail : function( link, event ) {
    ...
    var tr = link.ancestors().find(function(element){
      return Paginate.Util.isRecordRow(element);
    })
...
Object.extend(Paginate.Page, {
  /**
   *  其上下文对象说明了创建的模型名称和url
   */
  create : function(){
    try{
      var element = $$("div.paginate").first();
      this.page = new Paginate.Page(element);
      Paginate.Page['instance'] = this.page;
    }catch(error){
      alert(error); //Here is the alert source
    }
  },
  getInstance : function(){
    return Paginate.Page.instance;
  },
  wrapEventListener: function(source, target) {
    return function(event){ //event was used in eval context
      eval(target);
    }
  }
})

Paginate.Table = Class.create({
  initialize: function(page, element) {
    ...
    //初始化Group Model
    this.groupModel = new Paginate.GroupModel(this);
    //初始化Column Model
    this.columnModel = new Paginate.ColumnModel(this);
    //创建选择模型
    if( this.element.readAttribute("selection") == "single" ){
      this.selectionModel = new Paginate.SingleSelectionModel(this);
    }else if( this.element.readAttribute("selection") == "none" ){
      this.selectionModel = new Paginate.NoneSelectionModel(this);
    }else{
      this.selectionModel = new Paginate.MultipleSelectionModel(this);
    }
    this.paginator = this.page.element.select("table.statusbar td.app_table_paginator").first();
    //Lock用于标记本表格是否正在抓取数据
    this.lock = false;
  ...
  /**
   * 为了减少交互次数，将最新的数据和分页信息一次性的传递过来，中间用
   * 一行特殊标记： __paginator_spliter___ 隔开
   * 另外一种方法，是根据 http header里面的结果，动态改变 paginator，但太麻烦
...
/**
 * Paginate Table的列模型，确切的讲，是列头模型Column Headers Model
 * 它并不是一个传统意义上的MVC Model
 * 其实只是Paginate Table一部分相关逻辑的汇集
 */
Paginate.ColumnModel = Class.create({
  initialize: function(table){
    this.table = table;
    this.columns = [];//ColumnHeader对象的数组，包括Selection Column Header
    var model = this;
    table.element.select("table.fixed_header th").each(function(th){
      model.columns.push(new Paginate.ColumnHeader(table, th))
    })
  },
  refresh : function(){
...
Paginate.ColumnHeader = Class.create({
 ...
 */
Paginate.SelectionModel = Class.create({
  initialize: function(table){
    this.table = table;
    this.selection = [];
  },
...
Paginate.MultipleSelectionModel = Class.create(Paginate.SelectionModel, {
  /*
   * 选择某个Button的值
   **/
...
//系统生成的对象
Paginate.SingleSelectionModel = Class.create(Paginate.SelectionModel, {
  /*
   * 选择某个Button的值
   **/
  select : function(value){
...
Paginate.NoneSelectionModel = Class.create(Paginate.SelectionModel, {
  select : function(){},
  unselect : function(){},
  checkNone: function(){},
  checkAll: function(){}
...
Paginate.Toolbar = Class.create({
  initialize: function(page, element) {
    ...
    this.element.select("td.tool_button button").each(function(td){
      toolbar.buttons.push(new Paginate.Button(toolbar, td))
    });
  },
...
Object.extend(Paginate.Button, {
  ...
  activateNew : function(button){
    Paginate.Button.activate(button);
  },
  activateRefresh : function(button){
    Paginate.Page.getInstance().reload();
  },
  activateEdit : function(button){
    var object_id = Paginate.Page.getInstance().getSelection()[0].strip();
  ...
  activateBatch:function(button, method){
    ...
    var page = Paginate.Page.getInstance();
  ...
  activateDelete : function(button){
    ...
    if( !confirm( promotion ) ){
      ...
      Paginate.Button.activateBatch(button, 'delete');
    }
  },
...
Paginate.GroupModel = Class.create({
  initialize: function(table){
    this.table = table;
    //分组条，默认为空
    this.groupbars = [];
  ...
  hook : function(){
    ...
    this.table.element.select("tr.group").each(function(tr_group){
      ...
      model.groupbars.push(new Paginate.Groupbar(model.table, tr_group));
...
Paginate.Groupbar = Class.create({
  initialize: function(table, element){
    this.table = table;
    this.element = $(element);
...
Paginate.Statusbar = Class.create({
  initialize: function(page, element) {
    this.page = page;
    this.element = $(element);
    this.items = [];
    ...
    this.element.select("td.status_item").each(function(li){
      statusbar.items.push(new Paginate.StatusItem(statusbar,li));
    })
  }
})

Paginate.StatusItem = Class.create({
